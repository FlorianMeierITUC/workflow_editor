...
import de.ketobi.vaadinspringdemo.apps.ausschreibung.entities.PendingDocumentFileBytes;
...
public class AusschreibungActionButtons extends HorizontalLayout {
        ...
        public AusschreibungActionButtons(Ausschreibung ausschreibung, AusschreibungService ausschreibungService,
                ...
                Button saveButton = new Button("Speichern & Weiter", e -> {
                        ...
                        if (tabs.getSelectedIndex() == 2) {
                                ...
                                spinner.getStyle().set("visibility", "visible");

                                // Either create or update project
                                Mono<ProjectResponse> projectMono = ausschreibung.getUuid() == null
                                                ? ausschreibungService.createProject(ausschreibung)
                                                : ausschreibungService.updateProject(ausschreibung);

                                projectMono.flatMap((ProjectResponse project) -> {
                                        ausschreibung.setUuid(project.getProjectUuid());

                                        return Flux.fromIterable(ausschreibung.getPendingDocumentsFileBytes())
                                                        .flatMap((PendingDocumentFileBytes doc) -> {
                                                                String fileName = doc.getFileName();
                                                                byte[] bytes = doc.getFileBytes();

                                                                return ausschreibungService
                                                                                .extractAusschreibungText(bytes,
                                                                                                fileName)
                                                                                .flatMap(extractedText -> ausschreibungService
                                                ...
                                                .subscribe(project -> {
                                                        ...
                                                        if (ui != null && ui.isAttached()) {
                                                                ui.access(() -> showConfirmationDialog(ausschreibung,
                                                                                ausschreibungService));
                                                        }
                                                }, error -> {
                                                        if (ui != null && ui.isAttached()) {
                                                                ui.access(() -> Notification.show(
                                                                                "Fehler beim Speichern der Ausschreibung: "
                                                                                                + error.getMessage(),
                                                                                5000, Notification.Position.MIDDLE));
                                                        }
                                                });
        ...
        private void showConfirmationDialog(Ausschreibung ausschreibung, AusschreibungService service) {
                ...
                dialog.setHeaderTitle("Das Ausschreibungsprojekt " + ausschreibung.getTitle()
